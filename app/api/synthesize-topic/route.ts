...
export async function POST(request: NextRequest) {
  try {
    const { topic, contentFragments } = await request.json();
    ...
    const prompt = `You are synthesizing multiple user responses into a single, coherent professional paragraph.
...
**Output Format:**
Return ONLY the synthesized paragraph as plain text, without any JSON wrapper or metadata.`;
